...
# Target output
TARGET_ELF := $(BUILD_DIR)/target/aarch64-sel4-microkit-minimal/debug/pwm_driver.elf
...
$(TARGET_ELF): $(BUILD_DIR)
	@echo "Building pwm_driver.elf for board $(MICROKIT_BOARD)..."
...
$(IMAGE_FILE) $(REPORT_FILE): $(TARGET_ELF) ./pwm.system
	$(MICROKIT_TOOL) ./pwm.system --search-path $(BUILD_DIR)/target/aarch64-sel4-microkit-minimal/debug --board $(MICROKIT_BOARD) --config $(MICROKIT_CONFIG) -o $(IMAGE_FILE) -r $(REPORT_FILE)